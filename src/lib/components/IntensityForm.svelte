<script lang="ts">
	import type { Intensity, IntensityInput, FieldErrors } from '../types/intensity';
	...
	interface Props {
		...
		onSubmit: (data: IntensityInput) => void;
		onCancel: () => void;
	}

	let { initialData, isLoading = false, fieldErrors = {}, onSubmit, onCancel }: Props = $props();

	let formData = $state<IntensityInput>({ name: '' });

	$effect(() => {
		formData = initialData ? { name: initialData.name } : { name: '' };
	});
