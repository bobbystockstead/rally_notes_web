<script lang="ts">
	import type { Stage, StageInput, FieldErrors } from '../types/stage';
	...
	interface Props {
		...
		onSubmit: (data: StageInput) => void;
		onCancel: () => void;
	}

	let { initialData, isLoading = false, fieldErrors = {}, onSubmit, onCancel }: Props = $props();

	let formData = $state<StageInput>({ name: '', distance: null });

	$effect(() => {
		formData = initialData
			? { name: initialData.name, distance: initialData.distance }
			: { name: '', distance: null };
	});
