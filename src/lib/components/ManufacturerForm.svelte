<script lang="ts">
	import type { Manufacturer, ManufacturerInput, FieldErrors } from '../types/manufacturer';
	...
	interface Props {
		...
		onSubmit: (data: ManufacturerInput) => void;
		onCancel: () => void;
	}

	let { initialData, isLoading = false, fieldErrors = {}, onSubmit, onCancel }: Props = $props();

	let formData = $state<ManufacturerInput>({ name: '' });
