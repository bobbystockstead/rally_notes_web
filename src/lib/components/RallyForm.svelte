<script lang="ts">
	import type { Rally, RallyInput, FieldErrors } from '../types/rally';
	...
	interface Props {
		...
		onSubmit: (data: RallyInput) => void;
		onCancel: () => void;
	}

	let { initialData, isLoading = false, fieldErrors = {}, onSubmit, onCancel }: Props = $props();

	let formData = $state<RallyInput>({ name: '', date: null });

	$effect(() => {
		formData = initialData
			? { name: initialData.name, date: initialData.date }
			: { name: '', date: null };
	});
